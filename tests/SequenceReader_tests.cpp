...
TEST_CASE("All sequences are loaded") {
    ...
    const std::pair<SDB, std::set<Item>> result = sequence_reader.read_dataset("../tests/data/sample.data");
...
TEST_CASE("All items have profit value assigned") {
    SequenceReader sequence_reader;
    const std::pair<SDB, ProfitTable> result = sequence_reader.prepare_data_for_sequence_mining(
